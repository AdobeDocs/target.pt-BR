...
Os recursos do seu provedor de serviços de email determinam qual método utilizar. Seu gerente de conta ou consultor pode ajudá-lo a escolher a opção que funciona melhor para você.

>[!IMPORTANT]
>
>As seguintes diretrizes de capacidade se aplicam às opções de modelo de email da API de entrega e do rawbox descritas abaixo (opções 1 e 2):
>
...
>* Aumente o tráfego em etapas de 200 solicitações por segundo a cada minuto

> 
>
Entre em contato com seu gerente de conta se desejar usar limites de taxa mais alta.

## Opção 1: Use a API de entrega {#section_9F00D271BABA4B7390B461F4C44EC319}
...
A maioria dos clientes de email não permite solicitações POST. Portanto, essa API não é recomendada para casos de uso em tempo aberto. Alguns clientes de email, como o Gmail ou o Outlook, podem armazenar o conteúdo em cache ou bloquear a imagem e exigir que o recipient permita que a imagem seja renderizada de forma proativa.
...
>[!NOTE]
>
>Ao usar um rawbox e [!DNL Target], consulte o aviso de segurança importante em [Criar listas de permissões que especificam hosts autorizados a enviar chamadas de mbox para [!DNL Target]](/help/administrating-target/hosts.md#allowlist).
...
* Nesse caso, a resposta é o que for definido como o valor do parâmetro `mboxDefault`. Veja a explicação abaixo sobre este parâmetro.
...
* Nesse caso, o servidor [!DNL Target] retorna o seguinte conteúdo:
...
* O aplicativo de email deve procurar esse texto e ser capaz de lidar com o erro. O provedor de email tem várias opções para lidar com este caso:
   ...
   * Coloque esse email específico na fila e execute novamente os emails com falha como um lote no final da execução inicial.
...
>[!NOTE]
>
>Para usar [!DNL Recommendations] no email, a chamada de rawbox geralmente deve incluir `entity.id` ou `entity.categoryId` ou ambas, dependendo do tipo de critério de recomendação. O exemplo de chamada acima inclui ambas.
...
| `mboxDefault` | *`https://www.default.com`* | Se o parâmetro `mboxNoRedirect` não estiver presente, `mboxDefault` deve ser um URL absoluto que retorna o conteúdo padrão se nenhuma recomendação estiver disponível. Esse URL pode ser uma imagem ou outro conteúdo estático.<br>Se o parâmetro `mboxNoRedirect` estiver presente, `mboxDefault` poderá ser qualquer texto indicando que não há recomendações, por exemplo `no_content`.<br>O provedor de email precisa lidar com o caso em que esse valor é retornado e inserir o HTML padrão no email.  <br> **Prática** recomendada de segurança: Se o domínio usado no  `mboxDefault` URL não for incluir na lista de permissões, você poderá ser exposto a um risco de Vulnerabilidade de Redirecionamento Aberto. Para evitar o uso não autorizado de links Redirecionadores ou `mboxDefault` por terceiros, o Adobe recomenda usar &quot;hosts autorizados&quot; para lista de permissões os domínios de URL de redirecionamento padrão. O Target usa hosts para lista de permissões domínios aos quais você deseja permitir redirecionamentos. Para obter mais informações, consulte [Criar Lista de permissões que especificam hosts autorizados a enviar chamadas de mbox para [!DNL Target]](/help/administrating-target/hosts.md#allowlist) em *Hosts*. |  |
...
| `mbox3rdPartyId` | *xxx* | Use essa opção se tiver sua própria ID de visitante personalizada para usar no direcionamento de perfil. |  |
