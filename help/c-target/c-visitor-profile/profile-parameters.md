...
Os atributos de perfil em [!DNL Adobe Target] são parâmetros específicos de um visitante. Estes atributos são armazenados no perfil do visitante para fornecer informações sobre ele que podem ser usadas em suas atividades.

Um perfil de usuário contém informações demográficas e comportamentais de um visitante da página da Web. Essas informações podem incluir idade, gênero, produtos comprados, última visita e assim por diante, que [!DNL Target] usa para personalizar o conteúdo que serve ao visitante.
...
1. Clique em **[!UICONTROL Públicos-alvo]** > **[!UICONTROL Scripts de perfil.]**
   ...
   ![Guia Scripts de perfil](/help/c-target/c-visitor-profile/assets/create-script.png)

1. Clique em **[!UICONTROL Criar script]**.

   ![Caixa de diálogo Criar scripts de perfil](/help/c-target/c-visitor-profile/assets/profile-script.png)
...
Por exemplo, o seguinte cartão de informações do script de perfil é acessado clicando no ícone [!UICONTROL Info] para o script de perfil desejado na lista ([!UICONTROL Públicos-alvo] > [!UICONTROL Scripts de perfil]).
...
>[!NOTE]
>
>A guia [!UICONTROL Uso do script] não exibe atividades que fazem referência ao script de perfil selecionado nas seguintes situações:
>
...
* Lembre-se não apenas do desempenho do script, mas do desempenho combinado de todos os scripts. Como prática recomendada, [!DNL Adobe] recomenda menos de 5.000 instruções no total. Contar o número de instruções não é óbvio, mas o importante a ser observado é que scripts que excedem 2.000 instruções são automaticamente desativados. O número de scripts de perfil ativos não deve exceder 300. Cada script é executado com cada chamada de mbox. Execute somente a quantidade de scripts necessária.
...
* As recomendações a seguir podem ajudar a limitar a complexidade do script de perfil. Os scripts de perfil podem executar um número limitado de instruções.
   ...
   * Evite a recursão.
   * Os scripts de perfil devem ser testados no desempenho antes de serem adicionados a [!DNL Target]. Todos os scripts de perfil são executados em cada solicitação de mbox. Se os scripts de perfil não forem executados corretamente, as solicitações da mbox levarão mais tempo para serem executadas, o que pode afetar o tráfego e a conversão.
...
>[!NOTE]
>
>Usar [!DNL console.log] em um script de perfil não resulta no valor do perfil, pois os scripts de perfil são executados no lado do servidor.

* **Adicione scripts de perfil como tokens de resposta para depurar scripts de perfil:**

   Em [!DNL Target], clique em **[!UICONTROL Administração]**, clique em **[!UICONTROL Tokens de Resposta]** e ative o script de perfil que deseja depurar.

   Sempre que você carregar uma página para seu site com [!DNL Target] nela, parte da resposta de [!DNL Target] conterá seu valor para o script de perfil específico, conforme mostrado abaixo:
