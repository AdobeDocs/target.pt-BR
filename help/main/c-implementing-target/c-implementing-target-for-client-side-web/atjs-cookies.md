...
| Duração do cookie | O cookie permanece no navegador do visitante por dois anos a partir do último logon.<br>O `deviceIdLifetime` pode ser substituída em [at.js versão 2.3.1 ou posterior](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-versions/){target=_blank}. Para obter mais informações, consulte [targetGlobalSettings()](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}. |
...
### Como o Adobe [!DNL Target] Trabalho de rastreamento?
