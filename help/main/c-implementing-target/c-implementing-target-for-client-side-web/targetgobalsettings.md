...
* **Descrição**: indica se deve-se usar o domínio `<clientCode>.tt.omtrdc.net` ou o domínio `mboxedge<clusterNumber>.tt.omtrdc.net`.
   ...
   Se este valor for true, o domínio `mboxedge<clusterNumber>.tt.omtrdc.net` será salvo em um cookie. Atualmente, não está funcionando com o [CNAME](https://developer.adobe.com/target/before-implement/implement-cname-support-in-target/){target=_blank} ao usar versões da at.js anteriores à at.js 1.8.2 e à at.js 2.3.1. Se isso for um problema para você, considere [atualização da at.js](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-versions/){target=_blank} para uma versão mais recente com suporte.
