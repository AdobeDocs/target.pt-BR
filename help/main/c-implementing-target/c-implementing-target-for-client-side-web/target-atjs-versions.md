...
description: Veja os detalhes das alterações em cada versão do Adobe [!DNL Target] Biblioteca de JavaScript da at.js.
...
## at.js 1.8.3 (21 de setembro de 2021) {#183}

Esta versão contém as seguintes alterações:

* Remoção do `reactor-window` e `reactor-document` [!DNL Adobe Experience Platform Launch] para garantir que a [!DNL Platform Launch] criar funções corretamente para clientes que `window.default` ou `document-default` definido.
* A at.js 1.8.3 agora define explicitamente `Samesite=None` e `Secure` para garantir que os cookies de domínio de terceiros sejam definidos corretamente.
...
* Janela otimizada e globais de documentos que fazem referência à variável [!DNL Adobe Experience Platform] extensão. (TNT-37124)
...
* Feito o `deviceIdLifetime` configuração substituível via [targetGlobalSettings](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}. (TNT-36349)
...
* Correção de um problema ao usar a extensão [!DNL Target]  v2 e a extensão [!DNL Adobe Analytics] [!DNL Launch], [!DNL Target] atrasava a chamada [!DNL Analytics] `sendBeacon`. (TNT-36407, TNT-35990, TNT-36000)

## at.js versão 2.3.0 (25 de março de 2020)

Essa versão do at.js é uma versão de manutenção e inclui os seguintes aprimoramentos e correções:

* Suporte à configuração da Política de segurança de conteúdo nas tags SCRIPT e STYLE anexadas ao DOM da página ao aplicar ofertas do Target entregues. Os clientes podem definir `targetGlobalSettings.cspScriptNonce` e `targetGlobalSettings.cspStyleNonce` para que a at.js possa definir o script correspondente e as funções da tag de estilo em ofertas aplicadas. Consulte  [targetGlobalSettings](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank} para obter mais detalhes.
* Correção de um problema ao compilar a at.js com o compilador do Google Closure para a implantação do Google Tag Manager.
* Cookie de verificação da at.js renomeado de `check` para `at_check` para evitar colisões com as implementações dos clientes.
...
* **Registro de análises do cliente**: forneça aos clientes controle total sobre como enviar dados de análise para o Adobe Analytics, seja no cliente ou no servidor.
   ...
   Para obter mais informações, consulte [Registro de análises no cliente](/help/main/c-integrating-target-with-mac/a4t/before-implement.md#client-side) em *Antes de implementar*.
...
* **Enviar notificações**: permite aos desenvolvedores enviar notificações quando uma experiência é renderizada pelo seu código em vez de usar `applyOffer()` ou `applyOffers()`.
   ...
   Para obter mais informações, consulte [adobe.target.sendNotifications(options)](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/adobe-target-sendnotifications-atjs-21/){target=_blank}.
...
Para obter mais informações, consulte [Atualização da at.js 1.x para at.js 2.x](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-versions/){target=_blank} e [Funções da at.js](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/atjs-functions/){target=_blank}.

>[!NOTE]
>
>Se você precisar de suporte do Adobe Opt-in para a variável [Regulamento Geral sobre a Proteção de Dados](https://developer.adobe.com/target/before-implement/privacy/cmp-privacy-and-general-data-protection-regulation/){target=_blank} (GDPR){target=_blank}, você deve usar atualmente a at.js 1.7.0 ou a at.js 2.1.0.
...
Para obter mais informações sobre a aceitação em Adobe, consulte [Privacidade e Regulamento Geral sobre a Proteção de Dados](https://developer.adobe.com/target/before-implement/privacy/cmp-privacy-and-general-data-protection-regulation/){target=_blank} (GDPR){target=_blank}.
...
>[!NOTE]
>
>Se você precisar de suporte do Adobe Opt-in para o GDPR, use a at.js 1.7.0 ou 2.1.0.<br>Para obter uma lista de todas as versões, consulte [Detalhes da versão da at.js](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-versions/){target=_blank}.
...
* Os seguintes novos eventos estão disponíveis para ajudar no rastreamento, depuração e personalização das interações com a at.js:
   ...
   * CONTENT_RENDERING_REDIRECT
   ...
   Para obter mais informações, consulte [Eventos personalizados da at.js](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/atjs-custom-events/){target=_blank}.
...
* É possível aumentar uma solicitação de at.js com parâmetros adicionais provenientes de provedores de dados. Os provedores de dados devem ser adicionados a `window.targetGlobalSettings` sob o `dataProviders key`.
   ...
   Para obter mais informações, consulte [Provedores de dados](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}.
...
* Adição do tratamento de token de resposta. Para obter mais informações, consulte [Tokens de resposta](/help/main/administrating-target/response-tokens.md#concept_2B21B222F6A344D68CA5929817E836C4).
...
* Carregue a biblioteca do Target de forma assíncrona. Para obter mais informações, consulte [Perguntas frequentes sobre at.js](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-faq/target-atjs-faq/){target=_blank}.
...
* Adição da configuração `selectorsPollingTimeout`. Para obter mais informações, consulte [targetGlobalSettings()](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}.
* O formato do formulário de resposta de `getOffer()` foi alterado. Para obter mais informações, consulte [adobe.target.getOffer(options)](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/adobe-target-getoffer/){target=_blank}.
...
* Os nomes das mboxes agora podem conter caracteres especiais, incluindo &quot;E&quot; comercial (&amp;). 
   ...
   Para obter uma lista de caracteres especiais permitidos, consulte [Configurações da at.js](https://developer.adobe.com/target/implement/client-side/atjs/how-to-deployatjs/implement-target-without-a-tag-manager/){target=_blank}.
...
* Se você usar o Adobe Analytics como a fonte de relatórios da sua atividade, não será necessário especificar um servidor de rastreamento durante a criação da atividade usando a mbox.js versão 61 (ou posterior) ou a at.js versão 0.9.1 (ou posterior). A biblioteca at.js envia automaticamente os valores do servidor de rastreamento ao [!DNL Target]. Durante a criação da atividade, é possível deixar o campo [!UICONTROL Servidor de rastreamento] em branco na página [!UICONTROL Metas e configurações].
