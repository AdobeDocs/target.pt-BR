...
description: Use adobe.target.sendNotifications() para at.js enviar notificações para a [!DNL Target] borda quando uma experiência é renderizada e não usa applyOffer(s). (at.js.2.1 +)
title: Como uso a função adobe.target.sendNotifications()?
feature: at.js
role: Developer
...
**Observação**: Os seguintes caracteres são *not* permitido para `Request > notifications > mbox > name`:
