...
description: Siga as etapas necessárias para implementar o Analytics for [!DNL Target] (A4T) nas soluções da Adobe [!DNL Target] e da Adobe Analytics.
...
As seções a seguir descrevem as etapas necessárias para implantar essa integração no site se você planeja usar o Platform Web SDK:
...
Consulte [Implementar o Serviço de ID da Experience Cloud para Target](https://experienceleague.adobe.com/docs/id-service/using/implementation/setup-target.html?lang=pt-BR) na documentação do *Serviço de ID de visitante da Experience Cloud*.
...
Para migração, consulte [Migração para o AppMeasurement para JavaScript](https://experienceleague.adobe.com/docs/analytics/implementation/js/migrate-from-hcode.html?lang=pt-BR) no *Guia de Implementação do Analytics*.
...
É especialmente importante confirmar se esses valores correspondem em Aplicativos de página única (SPAs), nos quais a ordem das chamadas nem sempre é previsível.
