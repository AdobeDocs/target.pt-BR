...
Ao criar uma atividade A/B usando o fluxo de trabalho guiado de três etapas, é possível escolher o [!UICONTROL Alocar automaticamente para a melhor experiência] opção.
...
Um teste A/B normal em [!DNL Target] mostra somente comparações emparelhadas de desafiantes com controle. Por exemplo, se uma atividade tiver experiências: A, B, C e D, onde A é o controle, uma atividade normal [!DNL Target] O teste A/B compararia A versus B, A versus C e A versus D.
...
[!UICONTROL Alocação automática] aloca de forma inteligente novos visitantes para experiências até que o intervalo de confiança da melhor experiência não se sobreponha ao intervalo de confiança de qualquer outra experiência. Normalmente, esse processo pode produzir falsos positivos, mas [!UICONTROL Alocação automática] usa intervalos de confiança com base no [Desigualdade de Bernstein](https://en.wikipedia.org/wiki/Bernstein_inequalities_%28probability_theory%29) que compensa avaliações repetidas. Neste ponto, há um verdadeiro vencedor. Quando [!UICONTROL Alocação automática] para, desde que não haja uma dependência de tempo substancial para os visitantes que chegam à página, há pelo menos 95% de chance [!UICONTROL Alocação automática] retorna uma experiência cuja resposta verdadeira não é pior que 1% (relativa) menos que a resposta verdadeira da experiência vencedora.
...
80% do tráfego é distribuído usando [!UICONTROL Alocação automática] e 20% do tráfego é distribuído aleatoriamente. Quando um vencedor é identificado, todos os 80% do tráfego vão para ele, enquanto todas as experiências continuam recebendo tráfego como parte dos 20%, incluindo a experiência vencedora.
