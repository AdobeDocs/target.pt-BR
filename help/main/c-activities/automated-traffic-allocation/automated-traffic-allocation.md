...
Ao [criar uma atividade A/B](/help/main/c-activities/t-test-ab/t-test-create-ab/test-create-ab.md) usando o fluxo de trabalho guiado de três etapas, escolha a opção **[!UICONTROL Alocar automaticamente para a melhor experiência]** na página **[!UICONTROL Direcionamento]** (etapa 2).
...
Uma atividade de [!UICONTROL Alocação automática] reduz esse custo e a sobrecarga de determinar uma experiência vencedora. A [!UICONTROL Alocação automática] monitora o desempenho da métrica de meta de todas as experiências e envia mais novos participantes para as experiências de alto desempenho proporcionalmente. Há tráfego suficiente reservado para explorar as outras experiências. Você pode ver os benefícios do teste nos resultados, mesmo enquanto a atividade está em execução: a otimização e o aprendizado ocorrem paralelamente.
...
[!UICONTROL Alocação automática] executa todas as comparações implícitas entre experiências e produz um vencedor &quot;verdadeiro&quot;. Não há noção de experiência de &quot;controle&quot; no teste.

[!UICONTROL A Alocação automática] aloca de forma inteligente novos visitantes para experiências até que o intervalo de confiança da melhor experiência não se sobreponha ao intervalo de confiança de qualquer outra experiência. Normalmente, esse processo pode produzir falsos positivos, mas a [!UICONTROL Alocação automática] usa intervalos de confiança baseados na [Desigualdade de Bernstein](https://en.wikipedia.org/wiki/Bernstein_inequalities_%28probability_theory%29){target=_blank} que compensa avaliações repetidas. Neste ponto, há um verdadeiro vencedor. Quando a [!UICONTROL Alocação automática] é interrompida, desde que não haja uma dependência de tempo substancial para os visitantes que chegam à página, há pelo menos 95% de chance de que a [!UICONTROL Alocação automática] retorne uma experiência cuja resposta verdadeira não seja pior do que 1% (relativa) menos do que a verdadeira resposta da experiência vencedora.

## Quando usar as atividades de [!UICONTROL Alocação automática] versus [!UICONTROL Teste A/B] ou [!UICONTROL Automated Personalization] {#section_3F73B0818A634E4AAAA60A37B502BFF9}
...
Os termos a seguir são úteis quando falamos de [!UICONTROL Alocação automática]:
...
A lógica geral por trás da [!UICONTROL Alocação automática] incorpora o desempenho medido (como a taxa de conversão) e os intervalos de confiança dos dados cumulativos. Ao contrário de um teste A/B padrão, em que o tráfego é dividido igualmente entre as experiências, a [!UICONTROL Alocação automática] altera a alocação de tráfego entre as experiências.
...
* Alterando opções no painel &quot;[!UICONTROL Configurações avançadas]&quot;
...
Considere as seguintes informações ao trabalhar com a [!UICONTROL Alocação automática]:

### O recurso [!UICONTROL Alocação automática] funciona somente com uma configuração de métrica avançada: [!UICONTROL Incrementar contagem e manter usuário na atividade]

Não há suporte para as seguintes configurações avançadas de métrica: [!UICONTROL Contagem de Incremento], [!UICONTROL Usuário de Liberação], [!UICONTROL Permitir Reentrada e Contagem de Incremento] e [!UICONTROL Usuário de Liberação e Barra de Reentrada].
...
[!UICONTROL A Alocação automática] é boa para diferenciar entre experiências de alto desempenho (e encontrar um vencedor). Pode ser que algumas vezes não haja diferenciação suficiente entre as experiências de desempenho inferior.
...
Alguns fatores que podem ser ignorados durante um teste A/B padrão porque afetam todas as experiências igualmente não podem ser ignorados em uma atividade [!UICONTROL Alocação automática]. O algoritmo está sujeito às taxas de conversão observadas.
...
80% do tráfego é distribuído usando a [!UICONTROL Alocação automática] e 20% do tráfego é distribuído aleatoriamente. Quando um vencedor é identificado, 80% do tráfego é direcionado a ele, enquanto todas as experiências continuam a receber algum tráfego como parte dos 20%, incluindo a experiência vencedora.
...
### Qual a diferença entre a [!UICONTROL Alocação automática] e a [!UICONTROL Automated Personalization]?

O [!UICONTROL Automated Personalization] usa os atributos de perfil de cada visitante para determinar a melhor experiência. Ao fazer isso, ela não apenas otimiza, mas também personaliza a atividade para esse usuário.

A [!UICONTROL Alocação automática], por outro lado, é um teste A/B que produz um vencedor agregado (a experiência mais popular, mas não necessariamente a experiência mais eficaz para cada visitante).
...
Você pode usar a [!DNL Adobe Target] [Calculadora de Tamanho da Amostra](/help/main/c-activities/t-test-ab/sample-size-determination.md#section_6B8725BD704C4AFE939EF2A6B6E834E6) existente para obter uma estimativa de quanto tempo o teste dura. (Assim como no teste A/B tradicional, aplique a correção de Bonferroni se estiver testando mais de duas ofertas ou mais de uma métrica/hipótese de conversão.) Essa calculadora foi projetada para testes A/B tradicionais de horizonte fixo e fornece apenas uma estimativa. O uso da calculadora para uma atividade de [!UICONTROL Alocação automática] é opcional porque [!UICONTROL Alocação automática] declara um vencedor para você. Você não precisa escolher um ponto fixo no tempo para ver os resultados do teste. Os valores fornecidos são sempre estatisticamente válidos.

Experimentos internos [!DNL Adobe] encontraram o seguinte:

* Ao testar exatamente duas experiências, a [!UICONTROL Alocação automática] encontra um vencedor mais rapidamente do que o teste de horizonte fixo (ou seja, o intervalo de tempo sugerido pela calculadora de tamanho de amostra) quando a diferença de desempenho entre as experiências é grande. No entanto, a [!UICONTROL Alocação automática] pode exigir tempo extra para identificar um vencedor quando a diferença de desempenho entre as experiências é pequena. Nesses casos, os testes de horizonte fixo normalmente terminariam sem um resultado estatisticamente significativo.
* Ao testar mais de duas experiências, a [!UICONTROL Alocação automática] encontra um vencedor mais rapidamente do que o teste de horizonte fixo (ou seja, o período sugerido pela calculadora de tamanho de amostra) quando uma única experiência tem um desempenho muito melhor do que todas as outras experiências. Quando duas ou mais experiências &quot;vencedoras&quot; em relação a outras experiências, mas são intimamente relacionadas entre si, a [!UICONTROL Alocação automática] pode exigir tempo extra para determinar qual é superior. Nesses casos, os testes de horizonte fixo normalmente terminariam concluindo que as experiências &quot;vencedoras&quot; eram melhores do que as experiências de baixo desempenho, mas não tinham identificado qual era superior.

### Devo remover uma experiência com baixo desempenho de uma atividade de [!UICONTROL Alocação automática] para acelerar o processo de decisão de um vencedor?

Não há motivo para remover uma experiência com baixo desempenho. A [!UICONTROL Alocação automática] veicula automaticamente experiências de alto desempenho com mais frequência e experiências com baixo desempenho com menos frequência. Deixar uma experiência com baixo desempenho na atividade não afeta significativamente a velocidade para determinar um vencedor.
...
### Posso alterar a métrica de meta durante uma atividade de [!UICONTROL Alocação automática]? {#change-metric}
...
Não é sugerido usar a opção [!UICONTROL Redefinir Dados de Relatório] para atividades de [!UICONTROL Alocação Automática]. Embora ela remova os dados do relatório visíveis, essa opção não remove todos os registros de treinamento do modelo [!UICONTROL Alocação automática]. Em vez de usar a opção [!UICONTROL Redefinir Dados do Relatório] para atividades de [!UICONTROL Alocação Automática], crie uma nova atividade e desative a original. (Esta orientação também se aplica às atividades de [!UICONTROL Direcionamento automático] e [!UICONTROL Automated Personalization].)

### Como a [!UICONTROL Alocação automática] cria modelos em relação a ambientes?

A [!UICONTROL Alocação automática] cria modelos com base no comportamento de tráfego e conversão registrado somente no ambiente padrão. Por padrão, [!UICONTROL Produção] é o ambiente padrão, mas ele pode ser alterado em [!DNL Target] ([Administração > Ambientes](/help/main/administrating-target/environments.md)).
...
Não, a [!UICONTROL Alocação automática] considera o desempenho de toda a atividade.
...
A [!UICONTROL Alocação automática] usa decisão fixa pelos mesmos motivos que [!UICONTROL Teste A/B] para atividades são fixos. A alocação de tráfego funciona somente para novos visitantes.
...
Este vídeo monstra como criar um teste A/B usando o fluxo de trabalho orientado de três etapas do Target. [!UICONTROL Alocação automática] é discutida a partir de 4:45.
