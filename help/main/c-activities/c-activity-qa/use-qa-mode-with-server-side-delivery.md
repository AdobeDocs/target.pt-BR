...
A implementação padrão do Controle de qualidade de atividade é compatível com a transferência de parâmetros `qa_mode` por meio de parâmetros `pageUrl`. Essa abordagem é conveniente para padrão/ajax [!DNL Target] chamadas. No entanto, para chamadas de servidor para servidor, isso não é a melhor abordagem para um caso SDK do Adobe Mobile, quando `pageUrl` não está disponível.
...
| evaluateAsTrueAudienceIds | Lista de IDs | Lista vazia. | Lista de IDs de público-alvo que devem sempre ser avaliadas como verdadeiras no escopo do [!DNL Target] solicitação. |
| evaluateAsFalseAudienceIds | Lista de IDs | Lista vazia. | Lista de IDs de público-alvo que sempre devem ser avaliadas como falsas no escopo do [!DNL Target] solicitação. |
