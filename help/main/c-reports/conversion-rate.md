...
description: Saiba como o Adobe [!DNL Target] mostra e calcula a taxa de conversão, o aumento, a confiança e o intervalo de confiança para cada experiência.
...
A ilustração a seguir mostra o cabeçalho do gráfico para uma atividade de exemplo com os cabeçalhos [!UICONTROL Conversion Rate], [!UICONTROL Lift] e [!UICONTROL Confidence] destacados.
...
![imagem de detalhes da taxa de conversão](assets/conversion-rate-detail.jpg)
...
Dados de AOV, RPV e Vendas são exibidos para cada experiência se você inseriu uma mbox Fazer pedido (`orderConfirmPage`) e a selecionou como a mbox de conversão.
...
![imagem do conf_report](assets/conf_report.png) ![imagem do conf_report_detail](assets/conf_report_detail.png)
...
O *intervalo de confiança* é um intervalo de estimativas no qual o valor verdadeiro da métrica pode ser encontrado em um determinado nível de confiança. O Target sempre exibe intervalos de confiança de 95%. O intervalo de confiança aparece como um percentual aproximado cinza claro na coluna Índice de conversão. No exemplo abaixo, o intervalo de confiança para o incentivo da Experiência B é aproximadamente 15,65%.

![imagem de taxa_de_conversão](assets/conversion_rate.png)

**Exemplo:** o RPV observado de uma experiência é de $10, e seu **intervalo de confiança** de 95% é de $5 a $15. Desconhecido para nós, seu verdadeiro RPV é $12. Em seguida, se executarmos esse teste várias vezes, 95% do tempo do intervalo de confiança calculado conterá o valor _true_ do RPV de $12.
...
Para calcular essas métricas, baixe o arquivo do Excel [Calculadora de Confiança Completa](/help/main/assets/complete_confidence_calculator.xlsx) do Target para inserir o valor da atividade ou revise [Cálculos estatísticos em testes A/Bn](/help/main/c-reports/statistical-methodology/statistical-calculations.md).
...
* [Criar uma solicitação de Data Warehouse](https://experienceleague.adobe.com/docs/analytics/export/data-warehouse/t-dw-create-request.html)
...
## Por que [!DNL Target] recomenda usar os testes t de Welch? {#t-test}
...
A [!DNL Target] recomenda usar o [teste t de Welch](https://en.wikipedia.org/wiki/Welch%27s_t-test), pois eles exigem menos suposições do que alternativas como testes z e são o teste estatístico apropriado para fazer comparações emparelhadas de métricas de negócios (quantitativas) entre experiências de controle e experiências alternativas.
...
Dada esta normalidade da média, pode demonstrar-se que a estatística de ensaio a utilizar segue uma distribuição t, uma vez que é o rácio entre um valor normalmente distribuído (a diferença nos meios da métrica de negócio) e um termo de escala baseado numa estimativa dos dados (o erro-padrão da diferença nos meios). O **t-test** é então o teste de hipótese apropriado, considerando que a estatística de teste segue uma distribuição t.

### Por que outros testes não são usados

Um **teste z** é tecnicamente inapropriado porque, no cenário de teste A/B típico, o denominador da estatística de teste não é derivado de uma variação conhecida e, em vez disso, deve ser estimado a partir dos dados. No entanto, para tamanhos de amostra grandes o suficiente, o teste z e o teste t são idênticos.

**Testes qui-quadrados** não são usados porque são apropriados para determinar se há uma relação qualitativa entre duas variantes (ou seja, uma hipótese nula de que não há diferença entre variantes). Os testes T são mais apropriados para o cenário de _quantitativamente_ comparando métricas.

O **teste U de Mann-Whitney** é um teste não paramétrico, que é apropriado quando a distribuição de amostragem da métrica média de negócios (para cada variante) não é distribuída normalmente. No entanto, como discutido anteriormente, dada a magnitude do tráfego envolvido em testes online, o Teorema do Limite Central normalmente se aplica, e assim o teste t pode ser aplicado com segurança.

Métodos mais complexos como **ANOVA** (que generalizam testes t para mais de duas variantes) podem ser aplicados quando um teste tem mais de duas experiências (&quot;Testes A/Bn&quot;). No entanto, ANOVA responde à pergunta &quot;se todas as variantes têm a mesma média&quot;, enquanto no teste A/Bn típico, estamos mais interessados em _qual variante específica_ é a melhor. Portanto, em [!DNL Target], aplicamos testes t regulares comparando cada variante a um controle, com uma correção de Bonferroni para levar em conta comparações múltiplas.