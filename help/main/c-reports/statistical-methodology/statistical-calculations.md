...
Esta página documenta os cálculos estatísticos detalhados usados nos testes manuais de A/Bn em [!DNL Adobe Target]. As definições são fornecidas para Taxa de conversão, Intervalo de confiança da taxa de conversão, Aumento, Intervalo de confiança para aumento e Confiança.

![Relatório do Target mostrando a Taxa de conversão, o Incentivo médio e o Intervalo de confiança e a Confiança de uma atividade de Teste A/B.](/help/main/c-reports/statistical-methodology/img/target_report.png)
...
* A soma sobre unidades *i* depende da escolha da metodologia de contagem.
   ...
   * If *Visitantes* é usada como metodologia de contagem, cada unidade é um visitante único definido como um participante exclusivo na atividade durante a vida útil da atividade.
   * If *Visitas* é usada como metodologia de contagem, cada unidade é uma visita única definida como um participante exclusivo em uma experiência durante uma [!DNL Target] sessão (com uma `sessionId`). Quando a variável `sessionId` for alterada ou o visitante atingir a etapa de conversão, uma nova visita será contada.
   * If *Impressões da atividade* é usada como metodologia de contagem, cada unidade é uma impressão exclusiva definida como cada vez que um visitante carrega qualquer página da atividade.

## Intervalo de confiança da média/taxa de conversão
...
Ao executar experimentos, a taxa de conversão que observamos para uma determinada experiência é uma *estimativa* da taxa de conversão &quot;true&quot;. Para quantificar a incerteza nessa estimativa, podemos usar um intervalo de confiança. [!DNL Target] sempre relata um intervalo de confiança de 95%, o que significa que, a longo prazo, 95% dos intervalos de confiança calculados incluem a taxa de conversão verdadeira da experiência.
...
A última coluna mostra a confiança em um [!DNL Target] relatório. A confiança de uma experiência é uma probabilidade (denotada como uma porcentagem) de obter um resultado menos extrema do que aquele que é realmente observado, visto que a hipótese nula é verdadeira. Em termos de valores p, a confiança exibida é *1 - valor p*. Intuitivamente, maior confiança significa que é menos provável que a experiência de controle e não controle tenha taxas de conversão iguais.
